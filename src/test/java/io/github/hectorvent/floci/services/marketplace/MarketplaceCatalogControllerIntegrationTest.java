...
class MarketplaceCatalogControllerIntegrationTest {
    ...
    void cancelAndValidationMatchAwsErrorShapes() {
        ...
        given().header("Authorization", auth()).patch("/CancelChangeSet?catalog=AWSMarketplace&changeSetId=" + id)
                .then().statusCode(200).body("ChangeSetId", equalTo(id));
        given().contentType("application/json").header("Authorization", auth()).body("{}")
                .post("/ListEntities").then().statusCode(422).body("__type", containsString("ValidationException"));
    }
    ...
    void tagsAndPoliciesPersistOnEntity() {
        String cs = given().contentType("application/json").header("Authorization", auth())
                ...
                .post("/StartChangeSet").then().statusCode(200).extract().path("ChangeSetId");
        var response = given().header("Authorization", auth()).get("/DescribeChangeSet?catalog=AWSMarketplace&changeSetId=" + cs).then().extract().response();
        String arn = response.path("ChangeSet[0].EntityArn");
        given().contentType("application/json").header("Authorization", auth()).body("{\"ResourceArn\":\""+arn+"\",\"Tags\":[{\"Key\":\"env\",\"Value\":\"test\"}]}").post("/TagResource").then().statusCode(200);
        given().contentType("application/json").header("Authorization", auth()).body("{\"ResourceArn\":\""+arn+"\"}").post("/ListTagsForResource").then().statusCode(200).body("Tags[0].Key", equalTo("env"));
        given().contentType("application/json").header("Authorization", auth()).body("{\"ResourceArn\":\""+arn+"\",\"Policy\":\"{\\\"Version\\\":\\\"2012-10-17\\\"}\"}").post("/PutResourcePolicy").then().statusCode(200);
        given().header("Authorization", auth()).get("/GetResourcePolicy?resourceArn="+arn).then().statusCode(200).body("Policy", containsString("2012-10-17"));
    }
