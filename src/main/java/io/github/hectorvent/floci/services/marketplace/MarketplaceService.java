...
public class MarketplaceService implements Resettable {
    ...
    public ObjectNode batchDescribeEntities(JsonNode request, String region) {
        ...
        if (inputs.size() < 1 || inputs.size() > 20) {
    ...
    public ObjectNode startChangeSet(JsonNode request, String region) {
        ...
        if (request.hasNonNull("ChangeSetName")) {
            cs.put("ChangeSetName", request.path("ChangeSetName").asText());
        }
    ...
    public ObjectNode cancelChangeSet(String catalog, String id, String region) {
        ...
        if (!"PREPARING".equals(status) && !"APPLYING".equals(status)) {
            throw new AwsException("ResourceInUseException", "Change set cannot be cancelled in status " + status + ".", 423);
        }
    ...
    public ObjectNode catalogTagResource(JsonNode request) {
        ...
        Map<String,String> merged = new java.util.LinkedHashMap<>(); JsonNode old = tags.get(arn).orElse(null); if (old != null) {
            old.forEach(t -> merged.put(t.path("Key").asText(), t.path("Value").asText()));
        } incoming.forEach(t -> { String k=text(t,"Key",true); merged.put(k,text(t,"Value",true)); }); if (merged.size()>50) {
    ...
    public ObjectNode catalogUntagResource(JsonNode request) {
        String arn=text(request,"ResourceArn",true); ensureResource(arn); ArrayNode keys=requireArray(request,"TagKeys"); java.util.Set<String> remove=new java.util.HashSet<>(); keys.forEach(k->remove.add(k.asText())); ArrayNode out=mapper.createArrayNode(); tags.get(arn).ifPresent(old->old.forEach(t->{ if(!remove.contains(t.path("Key").asText())) {
            out.add(t.deepCopy());
        } })); tags.put(arn,out); return mapper.createObjectNode();
    }
    ...
    public ObjectNode getResourcePolicy(String arn) { if (arn==null||arn.isBlank()) {
        throw validation("ResourceArn is required.");
    } ensureResource(arn); JsonNode p=resourcePolicies.get(arn).orElseThrow(()->notFound("Resource policy",arn)); return mapper.createObjectNode().put("Policy",p.asText()); }
    public ObjectNode deleteResourcePolicy(String arn) { if (arn==null||arn.isBlank()) {
        throw validation("ResourceArn is required.");
    } ensureResource(arn); if(resourcePolicies.get(arn).isEmpty()) {
        throw notFound("Resource policy",arn);
    } resourcePolicies.delete(arn); return mapper.createObjectNode(); }
    ...
    public ObjectNode describeAssessment(JsonNode request, String region) {
        requireCatalog(text(request,"Catalog",true)); String id=text(request,"AssessmentIdentifier",true); JsonNode a=assessments.get(id).orElse(null); if(a==null && id.contains("/")) {
            a=assessments.get(id.substring(id.lastIndexOf('/')+1)).orElse(null);
        } if(a==null) {
            throw notFound("Assessment",id);
        } return (ObjectNode)a.deepCopy();
    }
    ...
    private void applyChangeSet(ObjectNode cs,String region){ cs.put("Status","APPLYING"); for(JsonNode c:cs.path("ChangeSet")){ String changeType=c.path("ChangeType").asText(); JsonNode ref=c.path("Entity"); String type=ref.path("Type").asText(); String id=ref.path("Identifier").asText(); if(changeType.toLowerCase(Locale.ROOT).startsWith("delete")){ entities.delete(id); continue; } ObjectNode e=entities.get(id).filter(JsonNode::isObject).map(n->(ObjectNode)n.deepCopy()).orElseGet(mapper::createObjectNode); e.put("EntityType",type); e.put("EntityIdentifier",id); e.put("EntityId",id); e.put("EntityArn",arn(region,"AWSMarketplace/"+arnEntityType(type)+"/"+id)); e.put("LastModifiedDate",Instant.now().toString()); JsonNode d=c.get("DetailsDocument"); if(d!=null&&!d.isNull()) {
        e.set("DetailsDocument",d.deepCopy());
    } JsonNode legacy=c.get("Details"); if(legacy!=null&&!legacy.isNull()){ e.put("Details",legacy.asText()); if(!e.has("DetailsDocument")){ try{e.set("DetailsDocument",mapper.readTree(legacy.asText()));}catch(Exception parseError){ LOG.debug("Legacy Marketplace Details is not JSON; preserving it as an opaque string.", parseError); } } } entities.put(id,e); } cs.put("Status","SUCCEEDED"); cs.put("EndTime",Instant.now().toString()); }
    ...
    private ObjectNode changeSet(String id){ if(id==null||id.isBlank()) {
        throw validation("ChangeSetId is required.");
    } JsonNode n=changeSets.get(id).orElseThrow(()->notFound("Change set",id)); return (ObjectNode)n.deepCopy(); }
    private JsonNode changeSummary(JsonNode cs){ ObjectNode n=mapper.createObjectNode(); for(String f:List.of("ChangeSetId","ChangeSetArn","ChangeSetName","Intent","StartTime","EndTime","Status")) if(cs.has(f)){
        n.set(f,cs.get(f));
    } return n; }
    ...
    private JsonNode assessmentSummary(JsonNode a){ ObjectNode n=mapper.createObjectNode(); for(String f:List.of("AssessmentArn","AssessmentId","FrameworkId","AssessmentResult","CreatedAt","ExpiresAt"))if(a.has(f)){
        n.set(f,a.get(f));
    } return n; }
    private ObjectNode page(String name,List<JsonNode> values,JsonNode req,int def,int min,int max){ int size=req.path("MaxResults").isInt()?req.path("MaxResults").asInt():def; if(size<min||size>max){
        throw validation("MaxResults is out of range.");
    } int off=decodeToken(req.path("NextToken").asText(null)); if(off>values.size()){
        throw validation("NextToken is invalid.");
    } int end=Math.min(values.size(),off+size); ObjectNode out=mapper.createObjectNode(); ArrayNode a=out.putArray(name); values.subList(off,end).forEach(v->a.add(v.deepCopy())); if(end<values.size()){
        out.put("NextToken",Integer.toString(end));
    } return out; }
    private static int decodeToken(String token){ if(token==null||token.isBlank()){
        return 0;
    } try{int i=Integer.parseInt(token);if(i<0){
        throw new NumberFormatException();
    }return i;}catch(Exception e){throw validation("NextToken is invalid.");} }
    ...
    private void ensureResource(String arn){ boolean exists=entities.scan(k->true).stream().anyMatch(e->arn.equals(e.path("EntityArn").asText()))||changeSets.scan(k->true).stream().anyMatch(c->arn.equals(c.path("ChangeSetArn").asText())); if(!exists){
        throw notFound("Resource",arn);
    } }
    private static ArrayNode requireArray(JsonNode n,String field){ JsonNode v=n.get(field); if(v==null||!v.isArray()){
        throw validation(field+" is required and must be an array.");
    } return (ArrayNode)v; }
    private static String text(JsonNode n,String field,boolean required){ JsonNode v=n==null?null:n.get(field); if(v==null||v.isNull()||!v.isTextual()||v.asText().isBlank()){if(required){
        throw validation(field+" is required.");
    }return null;} return v.asText(); }
    private static void requireCatalog(String c){ if(c==null||c.isBlank()){
