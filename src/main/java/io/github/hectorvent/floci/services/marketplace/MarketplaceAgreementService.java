...
public class MarketplaceAgreementService implements Resettable {
    ...
    private ObjectNode createAgreementRequest(JsonNode request) {
        ...
        if (!List.of("NEW", "AMEND", "REPLACE").contains(intent)) throw validation("intent must be NEW, AMEND, or REPLACE.");
        ArrayNode terms = array(request, "requestedTerms", true);
        if (terms.isEmpty() || terms.size() > 30) throw validation("requestedTerms must contain between 1 and 30 terms.");
        String source = text(request, "sourceAgreementIdentifier", false);
        String proposal = text(request, "agreementProposalIdentifier", false);
        if ("NEW".equals(intent) && source != null) throw validation("sourceAgreementIdentifier must not be provided for NEW intent.");
        if (!"NEW".equals(intent) && source == null) throw validation("sourceAgreementIdentifier is required for AMEND and REPLACE intents.");
        if (!"AMEND".equals(intent) && proposal == null) throw validation("agreementProposalIdentifier is required for NEW and REPLACE intents.");
        if (source != null) requireAgreement(source);
        String token = text(request, "clientToken", false);
        if (token != null) {
            for (JsonNode existing : agreementRequests.scan(k -> true)) {
                if (token.equals(existing.path("clientToken").asText(null))) return createAgreementRequestResponse(existing);
            }
        }
        ...
        if (source != null) record.put("sourceAgreementIdentifier", source); if (proposal != null) record.put("agreementProposalIdentifier", proposal);
        if (token != null) record.put("clientToken", token); record.put("createdAt", epoch()); record.put("status", "PENDING");
    ...
    private ObjectNode acceptAgreementRequest(JsonNode request) {
        ...
        if ("ACCEPTED".equals(ar.path("status").asText())) return mapper.createObjectNode().put("agreementId", ar.path("agreementId").asText());
        ...
        if (ar.has("agreementProposalIdentifier")) proposal.put("offerId", ar.path("agreementProposalIdentifier").asText());
    ...
    private ObjectNode searchAgreements(JsonNode request) {
        List<JsonNode> found=new ArrayList<>(); for(JsonNode a:agreements.scan(k->true)) if(matchesAgreementFilters(a, request.path("filters"))) found.add(agreementSummary(a));
        found.sort(Comparator.comparing(n->n.path("startTime").asDouble(0))); if("DESCENDING".equals(request.path("sort").path("sortOrder").asText())) java.util.Collections.reverse(found);
        return page("agreementViewSummaries",found,request,50);
    }

    private boolean matchesAgreementFilters(JsonNode a, JsonNode filters) {
        if (!filters.isArray()) return true;
        for (JsonNode f:filters) { String name=f.path("name").asText(); JsonNode values=f.path("values"); if(!values.isArray()||values.isEmpty()) continue; boolean any=false; for(JsonNode v:values){String wanted=v.asText(); String actual=switch(name){case "AgreementType"->a.path("agreementType").asText();case "Status"->a.path("status").asText();case "AcceptorAccountId"->a.path("acceptor").path("accountId").asText();case "ProposerAccountId"->a.path("proposer").path("accountId").asText();case "OfferId"->a.path("proposalSummary").path("offerId").asText();default->wanted;};if(wanted.equals(actual)){any=true;break;}} if(!any)return false; }
        return true;
    }

    private ObjectNode agreementSummary(JsonNode a){ObjectNode out=mapper.createObjectNode();for(String f:List.of("agreementId","agreementType","startTime","endTime","status"))if(a.has(f))out.set(f,a.get(f));copy(a,out,"acceptor");copy(a,out,"proposer");copy(a,out,"proposalSummary");return out;}

    private ObjectNode sendCancellation(JsonNode request) {
        String agreementId=text(request,"agreementId",true);requireAgreement(agreementId);String id="acr-"+compactId();ObjectNode r=mapper.createObjectNode().put("agreementCancellationRequestId",id).put("agreementId",agreementId).put("reasonCode",text(request,"reasonCode",true)).put("status","PENDING").put("createdAt",epoch()).put("updatedAt",epoch());if(request.hasNonNull("description"))r.put("description",request.path("description").asText());cancellationRequests.put(id,r);return r.deepCopy();
    }
    private ObjectNode getCancellation(JsonNode request){String agreementId=text(request,"agreementId",true);String id=text(request,"agreementCancellationRequestId",true);ObjectNode r=object(cancellationRequests,id,"Agreement cancellation request");if(!agreementId.equals(r.path("agreementId").asText()))throw notFound("Agreement cancellation request",id);return r;}
    ...
    private ObjectNode transitionCancellation(JsonNode request,String status,String statusMessage){ObjectNode r=getCancellation(request);r.put("status",status);r.put("updatedAt",epoch());if(statusMessage!=null)r.put("statusMessage",statusMessage);cancellationRequests.put(r.path("agreementCancellationRequestId").asText(),r);return r.deepCopy();}
    ...
    private ObjectNode sendPayment(JsonNode request){String agreementId=text(request,"agreementId",true);requireAgreement(agreementId);text(request,"termId",true);String id="pr-"+compactId();ObjectNode r=mapper.createObjectNode().put("paymentRequestId",id).put("agreementId",agreementId).put("status","PENDING").put("name",text(request,"name",true)).put("chargeAmount",text(request,"chargeAmount",true)).put("currencyCode",request.path("currencyCode").asText("USD")).put("createdAt",epoch());if(request.hasNonNull("description"))r.put("description",request.path("description").asText());paymentRequests.put(id,r);return r.deepCopy();}
    private ObjectNode getPayment(JsonNode request){String agreementId=text(request,"agreementId",true);String id=text(request,"paymentRequestId",true);ObjectNode r=object(paymentRequests,id,"Agreement payment request");if(!agreementId.equals(r.path("agreementId").asText()))throw notFound("Agreement payment request",id);return r;}
    private ObjectNode transitionPayment(JsonNode request,String status,String message){ObjectNode r=getPayment(request);r.put("status",status);r.put("updatedAt",epoch());if(message!=null)r.put("statusMessage",message);paymentRequests.put(r.path("paymentRequestId").asText(),r);return r.deepCopy();}
    ...
    private ObjectNode batchCreateBillingAdjustments(JsonNode request){ArrayNode entries=array(request,"billingAdjustmentRequestEntries",true);if(entries.isEmpty()||entries.size()>25)throw validation("billingAdjustmentRequestEntries must contain between 1 and 25 entries.");ObjectNode out=mapper.createObjectNode();ArrayNode items=out.putArray("items");out.putArray("errors");for(JsonNode e:entries){String agreementId=text(e,"agreementId",true);requireAgreement(agreementId);String id="bar-"+compactId();ObjectNode r=mapper.createObjectNode().put("billingAdjustmentRequestId",id).put("agreementId",agreementId).put("status","PENDING").put("adjustmentReasonCode",e.path("adjustmentReasonCode").asText("OTHER")).put("adjustmentAmount",e.path("adjustmentAmount").asText("0")).put("currencyCode",e.path("currencyCode").asText("USD")).put("createdAt",epoch()).put("updatedAt",epoch());if(e.has("description"))r.set("description",e.get("description"));billingAdjustments.put(id,r);items.add(r.deepCopy());}return out;}
    private ObjectNode getBillingAdjustment(JsonNode request){String agreementId=text(request,"agreementId",true);String id=text(request,"billingAdjustmentRequestId",true);ObjectNode r=object(billingAdjustments,id,"Billing adjustment request");if(!agreementId.equals(r.path("agreementId").asText()))throw notFound("Billing adjustment request",id);return r;}
    private ObjectNode listBillingAdjustments(JsonNode request){return page("items",billingAdjustments.scan(k->true),request,50);}

    private ObjectNode listAgreementCharges(JsonNode request){List<JsonNode> list=new ArrayList<>();for(JsonNode p:paymentRequests.scan(k->true)){if(request.hasNonNull("agreementId")&&!request.path("agreementId").asText().equals(p.path("agreementId").asText()))continue;ObjectNode charge=mapper.createObjectNode().put("agreementId",p.path("agreementId").asText()).put("chargeId",p.path("paymentRequestId").asText()).put("chargeAmount",p.path("chargeAmount").asText()).put("currencyCode",p.path("currencyCode").asText("USD"));list.add(charge);}return page("items",list,request,50);}
    ...
    private ObjectNode page(String field,List<JsonNode> values,JsonNode request,int defaultSize){int max=request.path("maxResults").isInt()?request.path("maxResults").asInt():defaultSize;if(max<1||max>100)throw validation("maxResults must be between 1 and 100.");int offset=token(request.path("nextToken").asText(null));if(offset>values.size())throw validation("nextToken is invalid.");int end=Math.min(values.size(),offset+max);ObjectNode out=mapper.createObjectNode();ArrayNode a=out.putArray(field);values.subList(offset,end).forEach(n->a.add(n.deepCopy()));if(end<values.size())out.put("nextToken",Integer.toString(end));return out;}
    private static int token(String value){if(value==null||value.isBlank())return 0;try{int n=Integer.parseInt(value);if(n<0)throw new NumberFormatException();return n;}catch(NumberFormatException e){throw validation("nextToken is invalid.");}}
    private ObjectNode requireAgreement(String id){return object(agreements,id,"Agreement");}
    private static ObjectNode object(AccountAwareStorageBackend<JsonNode> store,String id,String kind){if(id==null||id.isBlank())throw validation(kind+" identifier is required.");JsonNode n=store.get(id).orElseThrow(()->notFound(kind,id));if(!n.isObject())throw new AwsException("InternalServerException","Stored "+kind+" is invalid.",500);return (ObjectNode)n.deepCopy();}
    private static ArrayNode array(JsonNode request,String field,boolean required){JsonNode n=request.get(field);if(n==null||n.isNull()){if(required)throw validation(field+" is required.");return new ObjectMapper().createArrayNode();}if(!n.isArray())throw validation(field+" must be an array.");return (ArrayNode)n;}
    private static String text(JsonNode request,String field,boolean required){JsonNode n=request==null?null:request.get(field);if(n==null||n.isNull()||!n.isTextual()||n.asText().isBlank()){if(required)throw validation(field+" is required.");return null;}return n.asText();}
    private static List<JsonNode> nodes(JsonNode node){List<JsonNode> out=new ArrayList<>();if(node!=null&&node.isArray())node.forEach(out::add);return out;}
    private static void copy(JsonNode from,ObjectNode to,String field){if(from.has(field))to.set(field,from.get(field).deepCopy());}
