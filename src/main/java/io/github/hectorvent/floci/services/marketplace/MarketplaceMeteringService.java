...
public class MarketplaceMeteringService implements Resettable {
    private static final Pattern PRODUCT_CODE = Pattern.compile("[-a-zA-Z0-9/=:_.@]{0,255}");
    ...
    public MarketplaceMeteringService(StorageFactory factory, ObjectMapper mapper,
                                      RequestContext context, MarketplaceService catalog) {
        this.mapper = mapper;
        this.context = context;
        this.catalog = catalog;
        this.meterRecords = factory.create("marketplace", "marketplace-metering-records.json", type());
        this.clientTokens = factory.create("marketplace", "marketplace-metering-client-tokens.json", type());
        this.customerMappings = factory.create("marketplace", "marketplace-metering-customers.json", type());
    }
    ...
    public Response handle(String action, JsonNode request, String region) {
        JsonNode result = switch (action) {
            case "BatchMeterUsage" -> batchMeterUsage(request);
            case "MeterUsage" -> meterUsage(request);
    ...
    private synchronized ObjectNode meterUsage(JsonNode request) {
        ...
        String key = "meter/" + productCode + "/" + accountId() + "/" + dimension + "/" + hour;
        ...
        if (clientToken != null) {
            ...
            clientTokens.put("meter/" + clientToken, token);
        }
        return mapper.createObjectNode().put("MeteringRecordId", recordId);
    }

    private synchronized ObjectNode batchMeterUsage(JsonNode request) {
        JsonNode recordsNode = request.get("UsageRecords");
        if (recordsNode == null || !recordsNode.isArray() || recordsNode.size() > 25) {
            throw new AwsException("InvalidUsageAllocationsException", "UsageRecords must be an array with at most 25 records.", 400);
        }
        ...
        for (JsonNode record : recordsNode) {
            ...
            String key = "batch/" + digest(productIdentity + "|" + customerIdentity + "|" + dimension + "|"
    ...
    private ObjectNode registerUsage(JsonNode request, String region) {
        ...
        if (publicKeyNode == null || !publicKeyNode.canConvertToInt() || publicKeyNode.asInt() < 1) {
        ...
        String nonce = optionalText(request, "Nonce");
        if (nonce != null && nonce.length() > 255) {
            throw new AwsException("InvalidProductCodeException", "Nonce must be at most 255 characters.", 400);
        }
        String material = productCode + "|" + publicKeyVersion + "|" + (nonce == null ? "" : nonce)
                + "|" + accountId() + "|" + region;
        ObjectNode response = mapper.createObjectNode();
        response.put("Signature", Base64.getEncoder().encodeToString(sha256(material)));
        return response;
    }
    ...
    private synchronized ObjectNode resolveCustomer(JsonNode request, String region) {
        ...
        long number = 0;
        for (int i = 0; i < 6; i++) number = (number << 8) | (hash[i] & 0xffL);
        ...
        customerMappings.put(token, response);
    ...
    private void validateAllocations(JsonNode allocations, int quantity) {
        ...
        long sum = 0; Set<String> tagSets = new HashSet<>();
